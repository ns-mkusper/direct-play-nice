...
pub(in crate::subtitle_ocr) const PPOCR_V5_EN_REC_MODEL: ModelSpec = ModelSpec {
    filename: "en_PP-OCRv5_rec_mobile_ir9.onnx",
    url: "https://www.modelscope.cn/models/RapidAI/RapidOCR/resolve/v3.8.0/onnx/PP-OCRv5/rec/en_PP-OCRv5_rec_mobile.onnx",
    // Source ONNX uses IR v10; this checksum is for the IR-v9 compatibility
    // copy generated by only lowering the metadata IR version so ORT 1.16 can
    // load it on Maxwell/CUDA-12/cuDNN-8 runtimes.
    sha256: "640E323AC1A4DA562EDDD2A3963B4E1336686F898113983122D76D24DC3AFCFE",
};
...
pub(in crate::subtitle_ocr) fn ensure_ppocr_models(
    ...
    let rec = resolve_english_rec_model(model_dir, variant, rec_spec)?;
    Ok(PpOcrModels { det, cls, rec })
}

fn resolve_english_rec_model(
    model_dir: &Path,
    variant: PpOcrVariant,
    default_spec: &ModelSpec,
) -> Result<PathBuf> {
    if let Some(path) = env::var_os("DPN_OCR_REC_ENGLISH_MODEL") {
        let rec_path = PathBuf::from(path);
        if !rec_path.is_file() {
            bail!(
                "DPN_OCR_REC_ENGLISH_MODEL is set but file does not exist: '{}'",
                rec_path.display()
            );
        }
        info!(
            "{} english rec model override: '{}'",
            variant.label(),
            rec_path.display()
        );
        return Ok(rec_path);
    ...
    if env_flag_enabled("DPN_OCR_USE_PPOCRV5_EN_REC") {
        let candidate = model_dir.join(PPOCR_V5_EN_REC_MODEL.filename);
        if candidate.is_file() {
            info!(
                "{} english rec model override: using PP-OCRv5 English recognizer at '{}'",
                variant.label(),
                candidate.display()
            );
            return Ok(candidate);
        ...
        bail!(
            "DPN_OCR_USE_PPOCRV5_EN_REC=1 requires '{}' in OCR model dir (source URL: {}). The upstream ONNX must be converted to IR v9 for ORT 1.16 Maxwell compatibility before use.",
            PPOCR_V5_EN_REC_MODEL.filename,
            PPOCR_V5_EN_REC_MODEL.url
        );
    }

    ensure_model_file(model_dir, default_spec)
}
