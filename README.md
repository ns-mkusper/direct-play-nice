...
- On Linux, the CUDA EP must match the `libonnxruntime.so` build.
  Run `./scripts/ocr-tools/check_gpu_env.sh` (or `ldd` against the
  ONNX Runtime `.so`) to confirm which `libcudnn.so.*` is required
  and that it is discoverable
