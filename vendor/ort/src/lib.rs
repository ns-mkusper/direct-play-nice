...
static G_ORT_REQUESTED_API_VERSION: OnceLock<u32> = OnceLock::new();

/// Returns the API version requested from the loaded ONNX Runtime dynamic library.
///
/// When using a statically linked backend, this is equivalent to [`MINOR_VERSION`].
pub fn requested_api_version() -> u32 {
	*G_ORT_REQUESTED_API_VERSION.get_or_init(|| MINOR_VERSION)
}
		...
		.get_or_init(|| {
			...
			unsafe {
				...
				match lib_minor_version.cmp(&MINOR_VERSION) {
					Ordering::Less => crate::warn!(
						"ort {} loaded an older ONNX Runtime binary at `{}`; expected 1.{MINOR_VERSION}.x, got '{version_string}'. Using backward-compatible API request.",
				...
				let requested_api_version: u32 = core::cmp::min(lib_minor_version, ort_sys::ORT_API_VERSION);
				let _ = G_ORT_REQUESTED_API_VERSION.get_or_init(|| requested_api_version);
				let api: *const ort_sys::OrtApi = ((*base).GetApi)(requested_api_version);
				ApiPointer(NonNull::new(api.cast_mut()).unwrap_or_else(|| panic!(
					"Failed to initialize ORT API (requested API version {}, runtime {})",
			...
			unsafe {
				let _ = G_ORT_REQUESTED_API_VERSION.get_or_init(|| ort_sys::ORT_API_VERSION);
