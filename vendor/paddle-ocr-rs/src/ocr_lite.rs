...
impl OcrLite {
    ...
    pub fn init_models(
        ...
        self.db_net
            .init_model(det_path, num_thread, None)
            .map_err(|source| OcrError::ModelInit {
                stage: "det",
                source: Box::new(source),
            })?;
        self.angle_net
            .init_model(cls_path, num_thread, None)
            .map_err(|source| OcrError::ModelInit {
                stage: "cls",
                source: Box::new(source),
            })?;
        self.crnn_net
            .init_model(rec_path, num_thread, None)
            .map_err(|source| OcrError::ModelInit {
                stage: "rec",
                source: Box::new(source),
            })?;
    ...
    pub fn init_models_custom(
        ...
        self.init_models_custom_optional_cls(det_path, Some(cls_path), rec_path, builder_fn)?;
        Ok(())
    }

    pub fn init_models_custom_optional_cls(
        &mut self,
        det_path: &str,
        cls_path: Option<&str>,
        rec_path: &str,
        ...
        self.db_net
            .init_model(det_path, 0, Some(builder_fn))
            .map_err(|source| OcrError::ModelInit {
                stage: "det",
                source: Box::new(source),
            })?;
        if let Some(cls_path) = cls_path.filter(|v| !v.is_empty()) {
            self.angle_net
                .init_model(cls_path, 0, Some(builder_fn))
                .map_err(|source| OcrError::ModelInit {
                    stage: "cls",
                    source: Box::new(source),
                })?;
        } else {
            self.angle_net = AngleNet::new();
        }
        self.crnn_net
            .init_model(rec_path, 0, Some(builder_fn))
            .map_err(|source| OcrError::ModelInit {
                stage: "rec",
                source: Box::new(source),
            })?;
